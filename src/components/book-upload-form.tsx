...
export function BookUploadForm() {
  // Drive is the default/primary tab
  const [inputMode, setInputMode] = useState<'drive' | 'upload'>('drive')
  ...
  const handleDriveFilePicked = useCallback(
    (fileId: string, fileName: string) => {
      // Set picked file (private, read with token) — clears any URL input
  ...
  const handleTabChange = (value: string) => {
    setInputMode(value as 'drive' | 'upload')
    setError(null)
    setCoverFile(null)
  ...
  const canSubmit =
    ...
    ((inputMode === 'upload' && file) ||
     (inputMode === 'drive' && (driveValid || pickedFileId)))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (inputMode === 'drive' && pickedFileId) {
        // Private file from picker — read with user's token
        formData.set('drive_file_id', pickedFileId)
      } else if (inputMode === 'drive' && driveUrl) {
        // Public URL — read with proxy
          ...
          <TabsContent value="drive" className="mt-4 space-y-4">
            ...
            <Card>
              <CardHeader className="pb-3">
                ...
                <CardDescription>
                  Select a PDF from your Drive. No sharing required — we read it with your permission.
                </CardDescription>
              </CardHeader>
              <CardContent>
            ...
            {/* Section 2: Public link (separate flow) */}
            <Card>
              <CardHeader className="pb-3">
            ...
            {/* Cover upload — shows when a file is selected (picker or URL) */}
            <AnimatePresence>
                ...
                <CardContent className="space-y-4">
                  <div className="flex items-start gap-3">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => setUseFirstPageAsCover(checked === true)}
                    />
                    <div className="grid gap-1">
                      ...
                      <p className="text-muted-foreground text-xs">
                        The first page will be extracted and set as the book cover.
                      </p>
                    </div>
                  </div>
                  {pdfPreview && useFirstPageAsCover && (
                    <div className="flex items-start gap-4">
                      <div className="bg-muted aspect-[2/3] w-[120px] overflow-hidden rounded-md border">
                        <img src={pdfPreview} alt="First page preview" className="h-full w-full object-cover" />
                      </div>
                      <p className="text-muted-foreground text-xs">Preview of the first page</p>
                    </div>
                  )}
                  {!useFirstPageAsCover && (
                    <p className="text-muted-foreground text-xs">
